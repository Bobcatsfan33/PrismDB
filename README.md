...
- The deterministic local hash embedder remains the zero-configuration development default. S13's first production-plane increment now supports a separately supervised batched model service over a bounded Unix-socket protocol, with exact weights/tokenizer/preprocessing hashes persisted in each production generation and crash/reload fail-closed gates. The GPU service implementation, per-tenant policy/redaction/rate/cost controls, cache, and calibrated drift/OOD gates remain open; see [`docs/MODEL-PLANE.md`](docs/MODEL-PLANE.md).
