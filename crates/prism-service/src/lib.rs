//! Production read-only service boundary for PrismDB.
//!
...
impl ClientPolicy {
    fn validate(&self) -> Result<()> {
        ...
        if self.tenants.len() > MAX_TENANTS_PER_CLIENT
            || (self.permits("search") && self.tenants.is_empty())
        {
            return Err(invalid(format!(
                "identity `{}` needs 1..={MAX_TENANTS_PER_CLIENT} tenants when search is granted",
                self.identity_id
            )));
        }
        ...
        let allowed_scopes = ["health", "metrics", "search"];
        ...
        {
            return Err(invalid(format!(
                "identity `{}` scopes must be a non-empty subset of health, metrics, search",
                self.identity_id
            )));
        }
...
/// Search abstraction retained so the network and authorization boundary can be tested without
...
/// Mutual-TLS HTTP/1.1 server for tenant-scoped reads.
