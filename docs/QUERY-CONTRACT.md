...
- **What concurrent publication does:** nothing, to a pinned query. A shard's owner may publish new seqs while the query runs; the query keeps reading its pinned seq, and GC cannot reclaim it because the reader holds a **shard-local lease** on exactly the seqs it pinned — the distributed lease is the **conjunction of per-shard leases** ([D-075](DECISIONS.md); invariant 6, per shard), each protecting its snapshot for the lease-plus-derived-grace horizon. The lease clock is **monotonic**, so a wall-clock skew on a host reclaims nothing early and keeps nothing forever; the one unavoidable wall-clock comparison (against a persisted `created_at_ms`) is bounded by `MAX_CLOCK_SKEW_MS`, which the grace absorbs. A pinned seq reclaimed out from under a live reader — a reader past its lease, or crashed — is the expired-snapshot error (§2), never a wrong answer.
...
- **Opt-in partial results carry an explicit missing-shards report — and only ever on opt-in.** Best-effort completion is a **per-query** flag (`Query::best_effort`), never a global or a config default: a silently partial answer to a security or novelty question is the worst failure this product can produce, so opting in has to happen at the query. When a best-effort query drops a shard, the response carries a structured **`missing_shards`** report (`{shard, reason}` per shard) and mirrors the count into `counters.shards_missing`, so a degraded answer is a labelled, monitored number. If a shard disappears after exact scoring but before survivor materialization, every score from that shard is removed and the global top-`k` is finalized again, allowing reachable rows to backfill; returning the original top-`k` with missing bodies would be silently short. Because a fail-named query *errors* rather than returns, a non-empty `missing_shards` is **impossible to receive without having asked for it** — a partial answer can never be mistaken for a whole one. (A tenant-scoped query is all-or-nothing: its data lives on one shard, so there is no meaningful partial, and it fails-named if that shard is unreachable.)
- **Partial results and semantic aggregates do not mix — the aggregate is refused.** A best-effort **semantic `GROUP BY`** (or novelty) over an incomplete shard set is **refused by name**, not returned flagged. A cluster distribution is a function of the whole filtered set — cluster mass, exemplars, and drift baselines all shift with the data present — so a partial aggregate is not comparable to a complete run, and a flag on the response is too easy for an analyst reading a distribution to miss. Refusing is the only safe answer: the caller re-runs fail-named, or narrows to a reachable tenant. (A per-tenant aggregate routes to one shard and is unaffected.)
- **Hedged reads and retries are bounded and deduplicated** ([D-079](DECISIONS.md)). A slow shard may be hedged (its fragment re-issued); the hedge fan-out and the **in-flight blast-radius cap** are **[C-1](DECISIONS.md) constants**, not unbounded, so a slow cluster cannot hedge itself into collapse — past the cap, the query waits on the originals rather than amplifying load. A hedged fragment is **idempotent**: it reads a *pinned* seq (§19), so a re-issue is byte-identical, and the coordinator compares it to the original **bit-for-bit** and deduplicates by identity rather than racing two answers — a divergence is a named invariant violation, because a fragment must be deterministic for a hedge to be free. A gate asserts a hedged fragment and its original are identical, counted (`counters.hedges_issued`), and bounded by the cap, so hedging changes latency and never the answer. (The *timing* — when to hedge, how long a late duplicate is still a duplicate — is the asynchronous transport's, named and inert here.)
