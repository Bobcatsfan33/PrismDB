...
> - **OPEN — `threshold_overfetch` is not aggregated in a cluster.** §22 calls the overfetch “a monitored number”, but the counter is set only in the per-shard candidate phase and the coordinator builds its counters with `..Default::default()`, so **every cluster query reports 0**. Found while arming the prune gate, which therefore establishes its precondition on the single engine instead. Not fixed here — it is a behaviour change, and this increment was scoped to evidence.
...
Shard protocol v3 adds bounded, single-tenant ingest only to endpoints created
