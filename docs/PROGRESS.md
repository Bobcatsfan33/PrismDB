...
| **S11** | Object storage + local cache | cold/warm/thrash SLOs; cache corruption repaired from remote; rerank fetch bytes bounded by the plan | 🟡 **cold tier + publication + reconciliation + multipart green; catalog mirror (b) deferred** — the cold read runs through the ObjectStore+cache, and **answer-invariance forced cold/hot/mixed through MinIO under fault** is a permanent CI gate (byte-identical, transient→retry, dead→named); ack re-decided (D-068). **Boundary (a):** cold-tier remote-durable publication (upload→verify before reference, two kill-points driven old-or-new). **(c):** GC remote-orphan reconciliation (list+age+absent-from-catalog, mirror-aware, one `retained` set shared with local GC). **(d):** multipart PUT (initiate/part/complete/abort) + GC list-and-abort sweep. MinIO CI **pinned by digest** (D-070). **(b) CAS-on-`CURRENT` catalog mirror** is the remaining boundary — the object store is a mirror not a master (D-069), scoped as its own increment; the flip to ✅ awaits it plus the MEASURED cold-path worksheet ([#6](https://github.com/Bobcatsfan33/PrismDB/issues/6)) |
...
**Still 🟡, honestly.** The full directive-1 gate wants the kill-point matrix across the *upload / verify / CAS* boundaries and multipart-upload crash recovery, and directive 3 wants answer-invariance forced hot / cold / mixed *through the MinIO backend* — both need publication and the cold-tier read wired end-to-end through the `ObjectStore`, which is the remaining work in [#6](https://github.com/Bobcatsfan33/PrismDB/issues/6). The client is proven; the wiring is next.
...
**Still 🟡, and honestly so.** The remaining work to flip to ✅ is the *production publication path* (directive 2): `commit_meta` uploading and verifying a part's cold tier before it references it, with CAS on `CURRENT` and a kill-point at every arrow (upload / verify / CAS), plus GC reconciling remote orphans against the referenced set; multipart upload with list-and-abort recovery (directive 3); and the cost worksheet re-measured with cold-path request counts from the gate runs (directive 5). Tracked in [#6](https://github.com/Bobcatsfan33/PrismDB/issues/6). The read path of the truth now runs on object storage end-to-end and cannot be made to lie; the write path's remote-durable publication is the last piece.
