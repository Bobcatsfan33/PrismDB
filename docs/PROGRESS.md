...
| **S11** | Object storage + local cache | cold/warm/thrash SLOs; cache corruption repaired from remote; rerank fetch bytes bounded by the plan | 🟡 **client + MinIO gate green** — ObjectStore + cache + faults + fetch budget + cost worksheet, and the **hand-rolled SigV4 S3 client verified end-to-end against MinIO in CI** (round-trip, ranged get, CAS race, capability check); end-to-end cold-tier wiring + kill-point matrix across upload boundaries + multipart + answer-invariance-through-MinIO remain ([#6](https://github.com/Bobcatsfan33/PrismDB/issues/6)) |
