...
| **S11** | Object storage + local cache | cold/warm/thrash SLOs; cache corruption repaired from remote; rerank fetch bytes bounded by the plan | 🟡 **cold tier + answer-invariance through MinIO green** — the cold read runs through the ObjectStore+cache, and **answer-invariance forced cold/hot/mixed through MinIO under fault** is a permanent CI gate (byte-identical, transient→retry, dead→named); ack re-decided (D-068). Production publication (upload→verify→CAS **into commit** with kill-points), multipart, and GC remote-reconcile remain ([#6](https://github.com/Bobcatsfan33/PrismDB/issues/6)) |
