...
The cost worksheet now carries a **measured** cold-path request count (§7) alongside the projection, and the MinIO image is **pinned by digest** ([D-070](DECISIONS.md)) after a one-day-broken upstream (2025-09-06) was caught by the capability gate on the local fast loop. The honest walls remain named where they belong: TLS-over-WAN ([D-065](DECISIONS.md)) and a measured-at-768d worksheet against a real-embedding corpus ([#3](https://github.com/Bobcatsfan33/PrismDB/issues/3)) are the only deferrals, each scoped, neither claimed.
