...
- **Upload, then verify, then reference.** A part's cold tier is put to the store, and its presence and completeness are **verified** — a `head` confirming the object is durable and exactly the expected size — *before* the commit that names the part (`Engine::publish_part_cold`). The upload is idempotent (skipped when the backend already holds the object at that size, so the default local backend, rooted at the store, verifies what the part write already wrote). Verify failure is a refused publication, named, not a published-and-hope. **Byte-integrity is enforced per block on every read** (the CRC-32 framing, §4): an object stored at the right size but wrong bytes names itself the first time it is read rather than being served. *(The stronger publish-time content-hash agreement against the backend's own checksum — the S3 `ETag` — is filed with the S3-wire work, since it needs `head` to surface the backend checksum; what ships verifies presence+completeness and defers byte-integrity to the read-path CRC.)*
...
- **Orphans are GC's job, never publication's.** An upload that crashed, or a CAS that lost its race, leaves an orphaned remote object. It is reclaimed by **GC reconciling the remote listing against the referenced set** (`Engine::reconcile_remote_orphans`) — exactly as local GC reclaims a `.tmp` orphan — and **never** by the publication path deleting as it goes. Publication that cleans up is publication that can delete a live object on a retry. Reconciliation reclaims a remote object only when it is BOTH absent from the live set AND **older than the reader-lease horizon** (`GC_HORIZON_MS`): a just-uploaded cold tier whose commit has not yet landed is graced, never swept. It understands two remote schemes and shares **one definition of "live"** (`Catalog::retained`) with local GC: a cold tier `parts/<id>/rerank.vec` is live iff a retained snapshot references the part; a catalog mirror snapshot `catalog/SNAPSHOT-<snapshot_id>` ([D-069](DECISIONS.md)) is live iff its snapshot is retained — the invariant-6 grace-by-recovery-depth shape — so a snapshot the recovery path could need is never swept. An object under neither scheme is never touched.
