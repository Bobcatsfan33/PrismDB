...
- **What this session builds toward the mirror.** The mirror *writer* (the CAS + its kill-point at the rename→mirror boundary) is scoped as its own increment; what ships now is the *reconciler's* understanding of it: **remote-orphan reconciliation treats a `catalog/SNAPSHOT-<id>` object as live iff its snapshot is in the retained set** — the same invariant-6 shape as reader leases (grace by recovery depth) — so GC can never sweep a mirror snapshot the recovery path could need, the moment the writer lands.
